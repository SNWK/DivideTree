...
class AddNode(nn.Module):
    def __init__(self, graph_embed_func, node_hidden_size):
        ...
        self.node_type_embed = nn.Linear(4, node_hidden_size)
    ...
    def _initialize_node_repr(self, g, node_type, graph_embed):
        # node_type can store the information
        num_nodes = g.number_of_nodes()
        hv_init = self.initialize_hv(
            torch.cat((
                self.node_type_embed(torch.FloatTensor([node_type])),
                graph_embed), dim=1))
...
class AddEdge(nn.Module):
    ...
    def forward(self, g, action=None):
        ...
        if not self.training:
            # sample from trained Bernoulli distribution
            action = Bernoulli(prob).sample().item()
        ...
        # print('add edge', action, to_add_edge)
        return to_add_edge
...
class DGMG(nn.Module):
    ...
    def forward_train(self, actions):
        ...
        # if action == 1, stop = true
        stop = self.add_node_and_update(a=actions[self.action_step])

        while not stop:
            # if action == 0, to_add_edge = true
            to_add_edge = self.add_edge_or_not(a=actions[self.action_step])
            while to_add_edge:
                # dest = action, dest indicated by action, connect the latest one to the dest
