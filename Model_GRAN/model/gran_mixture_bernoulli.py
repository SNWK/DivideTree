...
class GRANMixtureBernoulli(nn.Module):
  ...
  def forward(self, input_dict):
    ...
    node_features = input_dict['node_features'] if 'node_features' in input_dict else None
    node_labels = input_dict['node_labels'] if 'node_labels' in input_dict else None
    ...
    self.use_mask_prob = input_dict[
        "use_mask_prob"] if "use_mask_prob" in input_dict else self.use_mask_prob
    ...
    if not is_sampling:
      ...
      features_list = [
          features[ii, :num_nodes[ii]] for ii in range(batch_size)
      ]
      labels_list = [
          labels[ii, :num_nodes[ii]] for ii in range(batch_size)
      ]
      return A_list, features_list, labels_list
