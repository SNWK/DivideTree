...
class GRANData(object):
  ...
  def __init__(self, config, graphs, tag='train'):
    ...
    # self.is_overwrite_precompute = True 
  ...
  def _get_graph_data(self, G):
    ...
    feature_0 = np.array([G.node[i]['feature'] for i in G.node])
    label_0 = np.array([G.node[i]['label'] for i in G.node])
    # test = list(G.node)[0]
    ...
    feature_1 = np.array([G.node[dd[0]]['feature'] for dd in degree_sequence])
    label_1 = np.array([G.node[dd[0]]['label'] for dd in degree_sequence])
    ### Degree ascent ranking
    degree_sequence = sorted(node_degree_list, key=lambda tt: tt[1])
    adj_2 = np.array(
        nx.to_numpy_matrix(G, nodelist=[dd[0] for dd in degree_sequence]))
    feature_2 = np.array([G.node[dd[0]]['feature'] for dd in degree_sequence])
    label_2 = np.array([G.node[dd[0]]['label'] for dd in degree_sequence])
    ...
    feature_5 = np.array([G.node[dd]['feature'] for dd in node_list])
    label_5 = np.array([G.node[dd]['label'] for dd in node_list])

    if self.num_canonical_order == 5:
      adj_list = [adj_0, adj_1, adj_3, adj_4, adj_5]
      node_feature_list = [feature_0, feature_1, feature_2, feature_3, feature_4, feature_5]
      node_label_list = [label_0, label_1, label_2, label_3, label_4, label_5]
    else:
      if self.node_order == 'degree_decent':
        adj_list = [adj_1]
        node_feature_list = [feature_1]
        node_label_list = [label_1]
      elif self.node_order == 'degree_accent':
        adj_list = [adj_2]
        node_feature_list = [feature_2]
        node_label_list = [label_2]
      elif self.node_order == 'BFS':
        adj_list = [adj_3]
        node_feature_list = [feature_3]
        node_label_list = [label_3]
      elif self.node_order == 'DFS':
        adj_list = [adj_4]
        node_feature_list = [feature_4]
        node_label_list = [label_4]
      elif self.node_order == 'k_core':
        adj_list = [adj_5]
        node_feature_list = [feature_5]
        node_label_list = [label_5]
      elif self.node_order == 'DFS+BFS':
        adj_list = [adj_4, adj_3]
        node_feature_list = [feature_4, feature_3]
        node_label_list = [label_4, label_3]
      elif self.node_order == 'DFS+BFS+k_core':
        adj_list = [adj_4, adj_3, adj_5]
        node_feature_list = [feature_4, feature_3, feature_5]
        node_label_list = [label_4, label_3, label_5]
      elif self.node_order == 'DFS+BFS+k_core+degree_decent':
        adj_list = [adj_4, adj_3, adj_5, adj_1]
        node_feature_list = [feature_4, feature_3, feature_5, feature_1]
        node_label_list = [label_4, label_3, label_5, label_1]
      elif self.node_order == 'all':
        adj_list = [adj_4, adj_3, adj_5, adj_1, adj_0]
        node_feature_list = [feature_4, feature_3, feature_5, feature_1, feature_0]
        node_label_list = [label_4, label_3, label_5, label_1, label_0]
      else:
        adj_list = [adj_0]
        node_feature_list = [feature_0]
        node_label_list = [label_0]
  ...
  def __getitem__(self, index):
    ...
    for ff in range(self.num_fwd_pass):
      ...
      data['node_features'] = np.array(node_feature_list)
      data['node_labels'] = np.array(node_label_list,dtype=np.int32)
  ...
  def collate_fn(self, batch):
    ...
    for ff in range(self.num_fwd_pass):
      ...
      data['node_features'] = torch.from_numpy(
          np.stack(
              [
                  np.pad(
                      bb['node_features'], ((0, 0), (0, pad_size[ii]), (0, 0)),
                      'constant',
                      constant_values=0.0) for ii, bb in enumerate(batch_pass)
              ],
              axis=0)).float()  # B X C X N X 3
      data['node_labels'] = torch.from_numpy(
          np.stack(
              [
                  np.pad(
                      bb['node_labels'], ((0, 0), (0, pad_size[ii])),
                      'constant',
                      constant_values=0.0) for ii, bb in enumerate(batch_pass)
              ],
              axis=0)).long()  # B X C X N X 1
