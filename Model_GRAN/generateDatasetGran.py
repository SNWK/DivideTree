...
def generate(nums, saddleSize, filepath):
    ...
    with open('data/DT/DS_A.txt', 'w+') as f:
        for edge in DS_A:
            f.write("{}, {}\n".format(edge[0], edge[1]))

    with open('data/DT/DS_graph_indicator.txt', 'w+') as f:
        for i in DS_graph_indicator:
            f.write("{}\n".format(i))
    
    with open('data/DT/DS_graph_labels.txt', 'w+') as f:
        for i in DS_graph_labels:
            f.write("{}\n".format(i))

    with open('data/DT/DS_node_labels.txt', 'w+') as f:
        for i in DS_node_labels:
            f.write("{}\n".format(i))

    with open('data/DT/DS_node_attributes.txt', 'w+') as f:
